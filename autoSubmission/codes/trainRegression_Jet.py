...
print("Train Config: ",trainConfig)
...
is_test = trainConfig.get("isTest", False)
...
snapshot_out_path = 'Snapshots/' + trainConfig.get("trainingName",datetime.today().strftime('%Y%m%d'))  # where to save the snapshots
...
out_path = out_path_eos + trainConfig["trainingName"] + '.root'


print("Input Path: ",in_path)
print("Tree Name: ",in_tree)
...
if is_test: num_epochs = 2
    ...
    num_epochs = trainConfig.get("nEpochs", 100)

learning_rate = 1e-5
lr_scheduler_gamma = 1.

if is_test: batch_size = 2048
else: batch_size = trainConfig.get("batchSize", 2048)


if is_test: num_batches = [2, 2, 2]
else: num_batches = trainConfig.get("numBatches", [500, 100, 200])
...
tanhmaskWithParameters = [int('tanh' in name[1]) for name in PARAMETERS + VARIABLES]
tanhmaskWithoutParameters = [int('tanh' in name[1]) for name in VARIABLES]
...
if any(tanhmaskWithParameters): model.add_module('TanhTransform', TanhTransform(mask=tanhmaskWithParameters, norm=tanhNorm))
...
if any(tanhmaskWithoutParameters): model.add_module('TanhTransformBack', TanhTransformBack(mask=tanhmaskWithoutParameters, norm=tanhNorm))
        ...
        for data_loader, benchmarks, start_points in zip([train_loader, validation_loader], [benchmarks_train, benchmarks_validation], [start_points_train, start_points_validation]):
            for batch, (inp, target, _) in enumerate(data_loader):
                ...
                if calculatelosseswithtransformedvariables:
                    ...
                    if any(tanhmaskWithParameters):
                        inp = TanhTransform(mask=tanhmaskWithParameters, norm=tanhNorm).forward(inp)
                    if any(tanhmaskWithoutParameters):
                        target = TanhTransform(mask=tanhmaskWithoutParameters, norm=tanhNorm).forward(target)
                        out = TanhTransform(mask=tanhmaskWithoutParameters, norm=tanhNorm).forward(out)
